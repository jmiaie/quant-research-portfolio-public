...
Each flagship project ships a **publication pack**: a self-contained directory of reports,
tables, figures, claim registers, and source maps, plus a verifier that re-derives every
...
Clone with full history (no `--depth`), then run from the repository root.
...
is **not** listed in the pack's `requirements.txt` at this commit — without it the run ends
...
— that is the check failing closed, not a broken artifact. Pack-pinned versions live in
...
claim-register citation resolves to a source. It does **not** assert out-of-sample trading
performance, and it is not a substitute for reviewing the methodology, the stated limitations,
or the `SOURCE-GATE.md` / `CLAIM-REGISTER.md` documents in each pack.
